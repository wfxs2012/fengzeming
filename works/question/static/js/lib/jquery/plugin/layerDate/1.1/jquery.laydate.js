...
//全局配置，如果采用默认均不需要改动
    var config = {
        ...
        path   : win.Nenu.CONSTANT.PATH.STATIC + 'js/lib/jquery/plugin/layerDate/1.1/' , //laydate所在路径
        skin   : 'default' , //初始化皮肤
...
//皮肤选择
    ...
    // fzm  add
    laydate.options = function ( opt ) {
        console.log ( Dates );
        if ( !Dates.options ) {
            Dates.options = options;
        }
        for ( var k in opt ) {
            Dates.options[k] = opt[k];
        }
    };
    // fzm add
    laydate.setDate = function ( ymd ) {
        
        Dates.elem = Dates.query ( options.elem );
        var ymd = ymd || Dates.elem[as.elemv].match ( /\d+/g ) || [];
        if ( !ymd.length ) {
            Dates.elem = null;
            return;
        }
        
        var elemIndexMap = Dates.getEachElementIndex ( Dates.options.format );
        
        ymd = [ymd[elemIndexMap.year] , ymd[elemIndexMap.month] , ymd[elemIndexMap.day]];
        
        var getDates = Dates.parse ( ymd , ['00' , '00' , '00'] );
