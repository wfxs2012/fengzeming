define ( function ( require ) {
    ...
    custom.setting = (function () {
        ...
        var _tools = {
            ...
            , resetColums : function ( json , index ) {
                ...
                for ( var i = 0; i < index; i++ ) {
                    var id = 'js_col_' + i + '_1_0';
                    newColums[id] = _colums[id];
                }

                _.assignIn ( newColums , json );
                _colums = newColums;
            }
        ...
        return {
            ...
            , validation : function () {
                ...
                var validMsg = $ ( '#' + key ).prev ();

                validMsg.hide ();

                if ( typeof(json.value) !== 'undefined' && !(json.value + '').replace ( /\s+/g , '' ).length ) {
                ...
                if ( typeof(json.label) !== 'undefined' && !json.label.replace ( /\s+/g , '' ).length ) {
                    msg.push ( '<li class="jc-col-valid-label">标题不能为空！</li>' );
                }

                if ( json.type === 'form' ) {

                    if ( typeof(json.fname) !== 'undefined' && !json.fname.replace ( /\s+/g , '' ).length ) {
                ...
                validMsg.html ( msg.join ( '' ) ).show ();

                this.shake ( {
                    element : json.type === 'form' ? validMsg[0] : $oldDrag.parent ()[0]
                } );

                setTimeout ( function () {
                    $ ( '#js_ctontent' ).stop ( true , true ).animate ( {
                        scrollTop : $oldDrag.parent ().position ().top + $ ( '#js_ctontent' ).scrollTop ()
                    } , 400 );
                } , 200 );
    ...
    custom.events = {
        ...
        save : function () {
            ...
            console.log ( custom.setting.validation () );

        }

        , control : function () {

            if ( !custom.setting.validation () ) {
                return;
            }
        ...
        , delItem : function () {
            ...
            var index = $item[0].id.split ( '_' )[2] | 0;
            ...
            $item.parent ().slideUp ( 300 , function () {
                ...
                custom.setting.resetColums ( tColumsJson , $item[0].id.split ( '_' )[2] | 0 );
                console.log ( custom.setting.getColums () );
